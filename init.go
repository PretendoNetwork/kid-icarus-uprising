...
func init() {
	...
	authenticationServerPort := os.Getenv("PN_KIU_AUTHENTICATION_SERVER_PORT")
	secureServerHost := os.Getenv("PN_KIU_SECURE_SERVER_HOST")
	secureServerPort := os.Getenv("PN_KIU_SECURE_SERVER_PORT")
	accountGRPCHost := os.Getenv("PN_KIU_ACCOUNT_GRPC_HOST")
	accountGRPCPort := os.Getenv("PN_KIU_ACCOUNT_GRPC_PORT")
	accountGRPCAPIKey := os.Getenv("PN_KIU_ACCOUNT_GRPC_API_KEY")
	friendsGRPCHost := os.Getenv("PN_KIU_FRIENDS_GRPC_HOST")
	friendsGRPCPort := os.Getenv("PN_KIU_FRIENDS_GRPC_PORT")
	friendsGRPCAPIKey := os.Getenv("PN_KIU_FRIENDS_GRPC_API_KEY")
	postgresURI := os.Getenv("PN_KIU_POSTGRES_URI")

	if strings.TrimSpace(postgresURI) == "" {
		globals.Logger.Error("PN_KIU_POSTGRES_URI environment variable not set")
		os.Exit(0)
	}
	...
	if strings.TrimSpace(authenticationServerPort) == "" {
		globals.Logger.Error("PN_KIU_AUTHENTICATION_SERVER_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(authenticationServerPort); err != nil {
		globals.Logger.Errorf("PN_KIU_AUTHENTICATION_SERVER_PORT is not a valid port. Expected 0-65535, got %s", authenticationServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(secureServerHost) == "" {
		globals.Logger.Error("PN_KIU_SECURE_SERVER_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(secureServerPort) == "" {
		globals.Logger.Error("PN_KIU_SECURE_SERVER_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(secureServerPort); err != nil {
		globals.Logger.Errorf("PN_KIU_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_KIU_SECURE_SERVER_PORT is not a valid port. Expected 0-65535, got %s", secureServerPort)
		os.Exit(0)
	...
	if strings.TrimSpace(accountGRPCHost) == "" {
		globals.Logger.Error("PN_KIU_ACCOUNT_GRPC_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(accountGRPCPort) == "" {
		globals.Logger.Error("PN_KIU_ACCOUNT_GRPC_PORT environment variable not set")
		os.Exit(0)
	}

	if port, err := strconv.Atoi(accountGRPCPort); err != nil {
		globals.Logger.Errorf("PN_KIU_ACCOUNT_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	} else if port < 0 || port > 65535 {
		globals.Logger.Errorf("PN_KIU_ACCOUNT_GRPC_PORT is not a valid port. Expected 0-65535, got %s", accountGRPCPort)
		os.Exit(0)
	...
	if strings.TrimSpace(accountGRPCAPIKey) == "" {
		globals.Logger.Warning("Insecure gRPC server detected. PN_KIU_ACCOUNT_GRPC_API_KEY environment variable not set")
	}
	...
	if strings.TrimSpace(friendsGRPCHost) == "" {
		globals.Logger.Error("PN_KIU_FRIENDS_GRPC_HOST environment variable not set")
		os.Exit(0)
	}

	if strings.TrimSpace(friendsGRPCPort) == "" {
		globals.Logger.Error("PN_KIU_FRIENDS_GRPC_PORT environment variable not set")
		os.Exit(0)
	}
	...
	if strings.TrimSpace(friendsGRPCAPIKey) == "" {
		globals.Logger.Warning("Insecure gRPC server detected. PN_KIU_FRIENDS_GRPC_API_KEY environment variable not set")
	}
